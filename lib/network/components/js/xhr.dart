...
/// 是否已初始化（幂等标记，存于 dartContext）
const XHR_INIT_KEY = "xhrInitialized";

/// 每个运行时的 XHR 轮询定时器句柄与"按需拉起"闭包。
/// 用 Expando（弱键）持有：运行时被回收后条目自动消失，也不把非序列化对象塞进
/// flutter_js 自己的 dartContext 里（上游 #674）。
final Expando<Timer> _xhrPollTimers = Expando<Timer>('xhrPollTimer');
final Expando<void Function()> _xhrPollStarters = Expando<void Function()>('xhrPollStarter');
...
extension JavascriptRuntimeXhrExtension on JavascriptRuntime {
  ...
  void disposeXhr() {
    _xhrPollTimers[this]?.cancel();
    _xhrPollTimers[this] = null;
    _xhrPollStarters[this] = null;
    dartContext[XHR_PENDING_CALLS_KEY] = [];
  ...
  void enableXhr2({bool enabledProxy = false}) async {
    ...
    final void Function() ensurePoller = () {
      final existing = _xhrPollTimers[this];
      if (existing != null && existing.isActive) return;

      _xhrPollTimers[this] = Timer.periodic(Duration(milliseconds: 20), (timer) {
    ...
    _xhrPollStarters[this] = ensurePoller;
    ensurePoller();
