...
- **唯一的例外，边界写死**（v1.24.23 起）：工具箱的「WAF 变异」支持载荷的**等价写法变异**与**受控主动探测**（① 识别 WAF → ② 挑技术 → ③ 分批试）。它刻意不做的是：不内置载荷字典、不做爆破 / 组合搜索、不并发（队列分批，需手动续批）、不自动利用、不反溯源；间隔 ≥ 100ms、总量 ≤ 2000 条两条底线封死；**每次探测必须显式勾选授权**。只对自有或已获书面授权的目标使用
...
- **它不做什么**：不构造载荷，没有扫描器、没有注入探测 / 爆破 / 绕过，不做自动利用。**默认不发送任何请求**——这是安全测试里的**被动扫描**，看已有的流量说话，回答"有哪些写法不安全"，而不是"能不能被打穿"。
- **可选：主动核验**（v1.24.30）：被动审计只能看你已抓到的那几条流量。点「主动核验」会对**已经在抓包里出现过的**域名各发一次 GET，核对 5 个安全响应头（HSTS / CSP / X-Content-Type-Options / X-Frame-Options / Referrer-Policy）。护栏：只限已抓域名、每个域名只发一次、串行 + 500ms 间隔、上限 50 个、开始前必须确认。不扫端口、不试协议、不投载荷。
- **可选：AI 分析**（v1.24.30）：把问题清单（**不含响应体原文**，敏感值已脱敏）交给 AI，拿回总结、优先级与修复建议。若 AI 建议调整 ProxyPin 自身设置，只能从白名单里挑（SSL 抓包 / 系统代理 / 防缓存 / MCP 局域网 / MCP 鉴权），**应用前一律再确认一次**。
...
- **填取值**：一行一个（`#` 开头的行忽略），或从**字典**一键填充——内置「边界值 / 类型异常」22 条种子（只读），也可手填 / 从 `.txt`·`.csv` 导入 / 用 JS 脚本生成；可开「先发基线」，后续每条都跟原始请求对比；
...
- **自动判定**（v1.24.29）：5 条「与基线比对」规则——请求失败（默认开）、状态码变化、长度明显变化（阈值 %）、响应明显变慢（阈值 ms）、命中关键字；命中的行挂标签。**填了参数才生效**，规则只标「和基线不一样」，不下漏洞结论；
- **间隔可调**（默认 200ms），随时可停。

**边界说清楚**：它**不提供成套攻击载荷库**（内置仅 22 条边界值 / 类型异常种子，取值由你自己填、导入或写脚本），不自动判定"有没有漏洞"，也不自动利用——**发什么由你决定，结论也由你下**。只对自己或已授权的目标用。
...
- **iOS 抓包更不容易被系统杀掉**（上游 #903）：单连接发送缓冲加了 4MB 上限、同时保持的连接数
  从 384 收到 192，并在扩展内存超过 45MB / 60MB 时按活跃度主动让位。
  表现上的差别就是：以前偶发"突然全网断、小窗也没了"，现在会先断几条连接自保。
- **自检页会告诉你证书装在哪一级**（上游 #200/#652/#728/#741）：如果只装进了"用户证书库"，
  Android 7 起应用默认不信任它 —— 这正是"证书明明装了却抓不到 HTTPS"最常见的原因，
  现在会直接点出来。装证书时记得选 **CA 证书**，别选成"VPN 和应用证书"。
- 顺带说清两个"看起来能修、其实修不了"的：
  **Flutter 应用抓不到**（它用自己打包的 CA 库，与系统信任库无关）、
  **Windows 上个别进程抓不到**（它们完全不走系统代理栈）。这两类需要改目标程序或上 TUN/驱动，
  不在代理工具的能力范围内。

## 本版新增（v1.22.96）

- **修好一个真会影响登录态的 bug**（上游 #901）：服务端一次返回多个 `Set-Cookie` 时，
  以前会被合并成一个字符串，浏览器会把它当成一个 Cookie 解析 —— 后者带
  `Max-Age=0` / `expires` 时会连带把前一个会话 Cookie 删掉。脚本里读
  `headers['set-cookie']` 也一并修正（多个 Cookie 现在用换行分隔，不再用分号硬拼）。
- 安卓批量导出不再谎报"导出成功：0 请求"（上游 #894），失败会告诉你原因，
  并建议改用「导出 HAR」。
- 批量重放的超时从 3 秒放宽到 10 秒（上游 #892），慢接口不再成批失败。
- **Linux arm64 的 .deb 现在真的能构建出来了**：改走 Flutter 官方交叉编译。

## 本版新增（v1.22.92）

- **WebSocket 终于能"动手"了**：在「WebSocket 拦截」规则里可以给帧配动作 ——
  改写内容、丢弃、延迟、重复发送。没配动作的规则行为跟以前完全一样。
- **Root 模式抓包（Android）**：设置 → Root 模式抓包。不开 VPN，用 root 权限直接接管
  系统流量，专治"一检测到 VPN 就拒绝联网"的应用。需要已 root；App 每次启动会清理上次
  异常退出的残留规则，不会让你莫名其妙上不了网。
- 桌面端与 Linux 的构建失败现在会如实标红，不再被静默吞掉。

## 本版新增（v1.22.91）

- **高流量下更顺了**：请求数到上限之后，旧版每个新请求都会重建一次整个列表，越忙越卡；
  现在只摘掉最旧的几条。顺手保住了你的筛选与选中状态。
- **重写规则的编辑器不再"打字乱跳"**：这是编辑器组件上游的光标跳动缺陷，本版换了修复版。
- **Ubuntu 装证书不再只会"安装失败"**：现在会依次尝试系统库、图形授权（pkexec）、
  以及浏览器真正信任的用户级证书库，失败时也会说明原因。
- 目标地址是 IP 的 HTTPS 站点（如 https://1.2.3.4:8081/）证书校验失败的问题也已修好。

## 本版新增（v1.22.90）

- **修掉 Android 上"个别请求莫名卡住 / 超时"的一个成因**：部分客户端会发送畸形或截断的
  TCP 选项，老代码解析这类选项时会抛异常，导致**这个包的后续处理被整体跳过**
  （数据没能推给目标服务器）。现在按规范逐项校验边界，畸形选项不再影响正常流程。
  这与上一版在 iOS 侧修的是同一处逻辑的两个平台实现。

## 本版新增（v1.22.89）

- **修好 iOS 上"多选导出 Request / Response 打不开分享面板"**（上游 #893）：
  以前在请求列表多选后导出 Request / Response / Request+Response，即使只选 1 条也会报
  `Is a directory, errno = 21`，系统分享面板根本弹不出来（导出 HAR 却正常）。
  现已改为先把内容写成真实文件再交给系统分享，导出后会正常弹出分享面板。
- **顺带清理了 iOS 扩展里约 40KB 从未被调用的代码**（老的 ObjC ping 实现等），安装包更小、链路更干净。
- **补了一处 HTTP/2 协议违规**：h2 规定 `te` 头只能取 `trailers`，以前会原样转发客户端的其它取值，
  严格的服务端可能因此拒绝请求。若你遇到"个别站点经抓包固定 403、关掉抓包就正常"，
  这处修复可能有用；但该问题成因不止一种，仍建议把该域名放进"不拦截"列表先恢复使用。

## 本版新增（v1.22.88）

- **修掉 Android 上「CA 根证书 读取失败」的误报**：以前 Android 的抓包自检里 CA 一项会显示
  `MissingPluginException`，看着像证书坏了——其实只是 Android 侧一直没实现这个检测方法。
  现在 Android 能真实判断"根证书装没装上"（读系统证书目录 + 用户凭据，**不需要 root**）。
- **Android 安装根证书的正确姿势**（很多人卡在"找不到证书文件"）：
  1. 设置 →「HTTPS 证书」→「导出 CA 证书」或「安装根证书 → 用户证书 → 下载根证书」，保存为 `ProxyPinCA.crt`；
  2. 再进系统设置 → 安全 → 加密与凭据 → **安装证书 → CA 证书**，选刚保存的文件。
  证书是内置在安装包里、首次运行才释放到应用私有目录的，所以在文件管理器里直接找是找不到的，
  **必须用 App 里的导出/下载按钮**拿。
  有 root 的话，同页「系统证书」Tab 里有「一键自动安装到系统」（Android 13 写 `/system/etc/security/cacerts`，
  14+ 写 `/apex/com.android.conscrypt/cacerts`），或用 Magisk 模块。
- **注意 Android 的平台限制**：Android 7 起，用户手动安装的 CA 默认不被普通应用信任，
  所以"装了用户证书还是抓不到 HTTPS"是正常现象；要全应用生效只有 root 装系统证书目录，或用 Xposed 模块。
- **导出证书失败不再静默**：以前在保存对话框点取消、或保存失败时没有任何提示，
  看起来就是"点了导出却没有文件"；现在会明确提示。
- **iOS：扩展内存现在可观测**（对应上游 #903）。VPN 扩展有自己独立的内存上限，超了会被系统直接杀掉
  （现象是网络突然全断、小窗消失、日志为空），而 App 里的「内存清理」管不到它。
  现在抓包自检页会显示扩展的内存水位（当前/峰值/连接数/待发缓冲），
  也可以用 Console.app 搜索 `extension memory` 看扩展周期性打印的水位日志。
...
- **Windows 上抓不到？先看「增强接管」**（上游 #896）：Windows 的系统代理只覆盖 WinINET，自带网络栈的程序（Electron/浏览器内核、部分 CLI、后台服务）不走它。偏好设置里的「Windows 增强接管」会额外接管 **WinHTTP** 与**用户级代理环境变量**，覆盖面更广——但它**默认关闭**：这两处都是**持久化**的（WinHTTP 属系统级、环境变量属用户级），万一程序来不及回滚，整机代理会一直指向已关闭的端口。默认不开，需要的场景手动打开即可。
- **残留代理会自愈**：即使上次异常退出留下残配置，启动时也会检查 WinHTTP 与 `HKCU\Environment` 里的代理变量——只有确实指向本机抓包端口时才清理，不会误伤你自己设的代理。
- **抓包自检页也会检查它**：工具箱 → 抓包自检会列出「Windows 增强接管」状态；未开启时直接说明哪些程序可能因此抓不到，并指路设置。
- **Root 模式的 iptables 更稳**（v1.23.0）：Android 上 iptables 的 xtables 锁是全局的（系统、其它模块、Magisk 服务都在抢），不加 `-w` 时会随机报「另一个程序正持有锁」，表现为「有时能开、有时失败」。现在所有规则都带 `-w`，会稍等重试而不是直接失败。

## 近期增强（v1.22.87 ~ v1.22.99）

- **iOS 抓包不再被内存拖垮**（上游 #903）：VPN 扩展有**独立于 App** 的内存上限，App 设置里的「内存清理」管不到它。现在扩展侧给每条连接的发送缓冲加了 4 MB 上限（超了断开该连接，避免无上限堆积）、连接数上限 384 → 192，并新增 45 MB / 60 MB 两级内存裁剪。同批还修了 TCP 序号回绕、ICMP 非对齐读取、零长度 UDP 数据报等 6 类会引发崩溃或静默的问题。
- **证书诊断更明确**：自检会区分「证书没装」「装在用户库但本 App 用系统库」「系统库只读（需 root / Magisk）」，并给出各自的做法。
- **一键安装系统证书改走 Magisk 模块**：现代 Android 的 `/system` 受 dm-verity 保护、`/apex` 是只读镜像，直接改写必然失败。现在改为生成 Magisk 模块由你刷入，并提供卸载入口。**非 root 设备只能装用户库证书**（Chrome 这类走系统库的 App 仍需 root 或改 apk）。
- **TUN 模式评估结论**：技术上可行但成本很高——瓶颈在桌面端没有 IP 协议栈（移动端那套是 Kotlin/Swift，Dart 无法复用），详见 `docs/tun_mode_evaluation.md`，优先级低。
